...
char *b64_encode_with_table(const char *table, const unsigned char *src, size_t len)
{
        int i = 0;
        int j = 0;
        char *enc = NULL;
...
{
        return b64_encode_with_table(b64_table, src, len);
}

char *b64url_encode(const unsigned char *src, size_t len)
{
        return b64_encode_with_table(b64url_table, src, len);
}
