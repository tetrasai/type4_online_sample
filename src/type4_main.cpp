/**
 * @file type4_main.cpp
 * @brief Tetras sample code which include DataRecv, DataAlign, SDK run,
 * DataTransmision
 * @version 0.1
 *
...
static void convertLPDResult(TS_LPD_result_t *lpd_result,
                             size_t lpd_result_len);
...
int main(int argc, char **argv) {
    ...
    tensor_file_dir =
        const_cast<char *>(root["inference_path"].string_value().c_str());
    image_file_dir =
        const_cast<char *>(root["raw_path"].string_value().c_str());

    auto mqtt_param_obj = root["mqtt_param"];
    std::string mqtt_broker_ip =
        mqtt_param_obj["mqtt_broker_ip"].string_value();
    std::string mqtt_broker_port =
        mqtt_param_obj["mqtt_broker_port"].string_value();
    ...
    if (!ZoomInParamObj.is_null()) {
        ...
        LOGI("zoomin x_magnify: %f, x_offset: %f, y_magnify: %f, y_offset: %f",
             x_magnify, x_offset, y_magnify, y_offset);
    }
    ...
    if (ret != TS_OK) {
        LOGE("TS_LPR_init failed, ret: %d", ret);
        TS_LPR_deinit();
        return -1;
    }

    LOGI("TS_LPR_init success");
    ...
    if (ret != TS_OK) {
        LOGE("TS_LPD_setParam failed, ret: %d", ret);
        TS_LPD_deinit();
        TS_LPR_deinit();
        return -1;
    }
    LOGI("TS_LPD_setParam success");
    ...
    if (!lpr_param_path.empty()) {
        ...
        ret = ReadFile(lpr_param_path.c_str(), &lpr_param_data,
                       &lpr_param_data_len);
    ...
    // data_trans.init(mqtt_broker_ip.c_str(), mqtt_broker_port.c_str(),
    // mqtt_sendbuf_size,
    //                 mqtt_recvbuf_size);
    ...
    while (true) {
        {
            ...
            lpd_result_len = data.lpd_result_len;
            {
                ...
                ret = TS_LPR_run(lpr_input, lpd_result, lpd_result_len,
                                 &lpr_result, &lpr_result_size);
            ...
            //         sprintf(img_save_path, "%lu_%06d.jpg", timestamp,
            //         img_count); cv::imwrite(img_save_path, mat);
            //     }
            // }
            ...
            //     data_trans.send_data(mqtt_topic.c_str(), gaze_result,
            //     gaze_result_len, gaze_input,
            ...
            LOGI("prepare data time cost: %ld ms, LPR time cost: %ld ms",
                 image_prepare_time_cost, lpr_time_cost);
...
static int ReceiveDataFromFile() {
    ...
    while (true) {
        ...
        while (true) {
            GetFileList(image_file_dir, image_list,
                        std::vector<std::string>{".nv12"});
            if (!image_list.empty()) {
                break;
            } else {
        ...
        LOGI("image num: %zu, queue_size: %zu", image_list.size(),
             g_data_queue.size());
        ...
        for (const auto &image_path : image_list) {
            ...
            tensor_path =
                tensor_path.substr(0, tensor_path.find_last_of("_")) + ".bin";

            std::string image_name = split_string(image_path, "/").back();
            std::string image_dir =
                image_path.substr(0, image_path.find_last_of("/"));
            std::string tensor_dir =
                tensor_path.substr(0, tensor_path.find_last_of("/"));

            // 20220428234546039_4032x3040.nv12
            std::string image_name_without_ext =
                split_string(image_name, ".").front();
            std::string width_str =
                split_string(image_name_without_ext, "x").front();
            width_str = split_string(width_str, "_").back();
            std::string height_str =
                split_string(image_name_without_ext, "x").back();
            ...
            // 1. read tensor data
            {
                ...
                ret = ReadFileByMMap(tensor_path.c_str(), &tensor_buf,
                                     &tensor_size);
                if (ret != 0) {
                ...
                int ret = TS_LPD_run(tensor_float, tensor_float_size,
                                     &lpd_result, &lpd_result_len);
                if (ret != TS_OK) {
...

            {
                if (delete_image_tensor) {
                    ...
                    if (pre_image_dir.empty()) {
                        ...
                        if (pre_image_dir.find(image_file_dir) !=
                            std::string::npos) {
                            RemoveFile(pre_image_dir);
                        }

                        if (pre_tensor_dir.find(tensor_file_dir) !=
                            std::string::npos) {
            ...
            int crop_width = (lpd_result[0].vehicle_bbox.right -
                              lpd_result[0].vehicle_bbox.left) *
                             x_magnify;
            int crop_height = (lpd_result[0].vehicle_bbox.bottom -
                               lpd_result[0].vehicle_bbox.top) *
                              y_magnify;
            // nv12 width and height adjust
            crop_width = (crop_width / 2) * 2;
            crop_height = (crop_height / 2) * 2;

            TS_LPR_image_t input_image = {(unsigned char *)nv_buf, width,
                                          height, 1, TS_PIX_FMT_NV12};
            TS_LPR_image_t output_image;
            output_image.format = TS_PIX_FMT_BGR888;
            {
                timeRecoder tr("===> crop and convert nv12 to bgr888");
                LOGI("crop_x: %d, crop_y: %d, crop_width: %d, crop_height: %d",
                     crop_x, crop_y, crop_width, crop_height);
                TS_LPR_image_crop_and_convert(input_image, &output_image,
...
static void convertLPDResult(TS_LPD_result_t *lpd_result,
    ...
    for (size_t i = 0; i < lpd_result_len; i++) {
        ...
        result.plate_bbox.left =
            (result.plate_bbox.left - result.vehicle_bbox.left) * x_magnify +
            x_offset;
        result.plate_bbox.top =
            (result.plate_bbox.top - result.vehicle_bbox.top) * y_magnify +
            y_offset;
        ...
        LOGI("after convert plate_bbox: %d, %d, %d, %d", result.plate_bbox.left,
             result.plate_bbox.top, result.plate_bbox.right,
             result.plate_bbox.bottom);

        result.vehicle_bbox.right =
            (result.vehicle_bbox.right - result.vehicle_bbox.left) * x_magnify +
            x_offset - 1;
        result.vehicle_bbox.bottom =
            (result.vehicle_bbox.bottom - result.vehicle_bbox.top) * y_magnify +
            y_offset - 1;
        result.vehicle_bbox.left = 0;
        result.vehicle_bbox.top = 0;

        LOGI("after convert vehicle_bbox: %d, %d, %d, %d",
             result.vehicle_bbox.left, result.vehicle_bbox.top,
             result.vehicle_bbox.right, result.vehicle_bbox.bottom);
    }
}
